...
class DetectSquares:
    ...
    def count(self, point: List[int]) -> int:
        ...
        for (qx, qy), cnt_q in self.point_cnt.items():
            ...
            p1 = (x, qy)
            p2 = (qx, y)
            res += cnt_q * self.point_cnt[p1] * self.point_cnt[p2]
        return res
...
count(): O(n), n = total stored points, iterate all points once

Space:
O(n), store all points in a hash map, space grows linearly with input size
"""